...
Also open: `make restore` reopens traffic right after reconcile without the acceptance check that restore.sh advises; restored files are root-owned (0644); impersonation human review (deferred by the owner; automated review only in 1.0), natural daily backup observation, the timed feasibility exercise, the weekly agent test, an independent repetition of the fourth-module exercise from published packages, killing a real worker process in the workflow crash test, and API contract coverage for the `./mcp`, `./provider`, `./commands` and `./eslint` exports.

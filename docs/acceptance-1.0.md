...
| 2 — Interface | Human RTL review, 100,000-row/50-user k6 budget, independent medical-assets slice with real minor upgrade and attachment restore, council 1 ADR | Accepted by owner attestation 2026-09-24 (ADR 025); the k6 deviation is re-measured in phase 5. |
| 3 — Business features | Policies, templates/SSE, comments/mentions/followers/tags/changes, assignments, saved views, import, printing, HMAC webhooks, API tokens/OpenAPI, 2FA with human ASVS review | Implemented and tested 2026-09-25 (XLSX import is GAP-006). 2FA moved to 2.0 and was removed from 1.0 (ADR 027). Waits on owner acceptance. |
| 4 — Workflows | Amend-by-copy, versioned XState definitions, row locks, six node types, durable transitions/retries, approval/run/retry screens, worker/definition-change tests | Implemented and tested 2026-09-25; waits on owner acceptance. |
| 5 — Agent/consumers | Isolated consumers for every capability, generated catalog, full idea review/five reviewers, module removal, second k6 and autonomous resource exercise | Catalog, five reviewers, agent module exercise and the CI k6 run are recorded; waits on owner acceptance. |
| 6 — Framework integration | API extraction, disposable independent consumer, customization/data preservation and Docker runtime/recovery verification, council 2 ADR | ADR 024 removes a permanent medical product from scope. API contract check and the genuine upgrade from published 0.2.0-alpha.1 run in CI; council 2 is proposed in ADR 026. |
